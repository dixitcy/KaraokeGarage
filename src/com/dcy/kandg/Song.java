...
import com.dcy.kandg.VocalTrack;
...
public class Song {
	...
	public HashMap<String, VocalTrack> vocalTracks; /*
													 * Vocal tracks...Key=Value
													 * pair
													 */
	...
	private static final String TAG = "KARAOKE_ENGINE_SONG";
	...
	/*
	 * void Song::dropNotes() { // Singing if (!vocalTracks.empty()) { for
	 * (VocalTracks::iterator it = vocalTracks.begin(); it != vocalTracks.end();
	 * ++it) it->second.notes.clear(); } b0rkedTracks = false; loadStatus =
	 * HEADER; }
	 */
	...
	/*
	 * Get a selected track, or LEAD_VOCAL if not found or the first one if not
	 * found
	 */
	...
	/*
	 * std::vector<std::string> getVocalTrackNames() { std::vector<std::string>
	 * result; BOOST_FOREACH(VocalTracks::value_type &it, vocalTracks) {
	 * result.push_back(it.first); } return result; }
	 */

	/** Get the song status at a given timestamp **/
	/*
	 * Song::Status Song::status(Number time) { Note target; target.end = time;
	 * Notes::const_iterator it =
	 * std::lower_bound(getVocalTrack().notes.begin(),
	 * getVocalTrack().notes.end(), target, noteEndLessThan); if (it ==
	 * getVocalTrack().notes.end()) return FINISHED; if (it->begin > time + 4.0)
	 * return INSTRUMENTAL_BREAK; return NORMAL; }
	 */

	public void print() {
		Log.d(TAG,"Check PRINT");
		Log.d(TAG, "Parsed file output:::::::");

		VocalTrack vocalTrack = getVocalTrack(LEAD_VOCAL);

		Log.d(TAG, "Song:::: BEGIN TIME: " + vocalTrack.beginTime
				+ " END TIME:" + vocalTrack.endTime);
		Log.d(TAG, "Song::::MIN NOTE:" + vocalTrack.noteMin + " MAX NOTE:"
				+ vocalTrack.noteMax);
